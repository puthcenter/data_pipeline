...
class IncrementalState:
    def __init__(self, G, initial_selection, ancestor_cache, externality_dict):
        ...
        self.active_list = list(initial_selection)
        self.active_idx_map = {n: i for i, n in enumerate(self.active_list)}
        
        self.ancestor_cache = ancestor_cache
        
        self.node_usage_counts = {}
        for r in ancestor_cache:
            for a in ancestor_cache[r]:
                self.node_usage_counts[a] = 0
    ...
    def batch_flip(self, nodes):
        ...
        for n in to_remove:
            ...
            idx = self.active_idx_map[n]
            last_node = self.active_list[-1]
            self.active_list[idx] = last_node
            self.active_idx_map[last_node] = idx
            self.active_list.pop()
            del self.active_idx_map[n]
        ...
        for n in to_add:
            ...
            self.active_idx_map[n] = len(self.active_list)
...
# ==============================================================================
# --- 3. 智能移动管理器 (附加 dataset_name 日志) ---
# ==============================================================================
class DynamicStructureManager:
    # 【改动】：传入 dataset_name，并为日志添加前缀
    def __init__(self, G, result_nodes, externality_dict, ancestor_cache, dataset_name="Unknown"):
        ...
        print(f"[{self.dataset_name}] [SmartMove] Analyzing Graph Structures...", flush=True)
    ...
    def _analyze_conflicts(self):
        ...
        print(f"[{self.dataset_name}]   -> Conflict Analysis: Threshold={threshold:.4f}, Retained Edges={count}/{len(penalties)}", flush=True)
        ...
        print(f"[{self.dataset_name}]   -> Identified {len(cliques)} strong conflict cliques.", flush=True)
        return cliques
    ...
    def _analyze_synergy(self):
        ...
        print(f"[{self.dataset_name}] [Synergy] Built index for {len(valid_results)} nodes.", flush=True)
        ...
        print(f"[{self.dataset_name}]   -> Synergy Analysis (Optimized): Threshold={SYNERGY_THRESHOLD}, Directed Links={strong_links}", flush=True)
        return followers
...
def get_smart_move(state: IncrementalState, manager: DynamicStructureManager, T: float, progress: float):
    ...
    if manager.synergy_followers and r < p_synergy:
        ...
        if seeds:
            ...
            return moves, 'Synergy'

    elif r < (p_synergy + p_conflict):
        if state.active_list:
            sample_size = min(len(state.active_list), 20)
            candidates = random.sample(state.active_list, sample_size)
            ...
            if victim and max_pen > 0:
                ...
                if active_enemies:
                    ...
                    if random.random() < 0.5: return [victim], 'Conflict'
                    else: return [enemy], 'Conflict'
        ...
        if len(active_in_clique) > 1:
            ...
            return [random.choice(clique)], 'Mutex'
        else:
            current = active_in_clique[0]
            target = random.choice(clique)
            if current != target:
                return [current, target], 'Mutex'
...
def run_solver(
    ...
    # 【改动】：添加日志前缀
    print(f"[{dataset_name}] [{algorithm_name.upper()}] Start... Budget: {max_runtime}s", flush=True)
    ...
    while time.time() < deadline:
        ...
        strategy_stats = {'Synergy': [0, 0], 'Conflict': [0, 0], 'Mutex': [0, 0], 'Random': [0, 0]}
        ...
        for _ in range(20):
            ...
            nodes, _ = get_smart_move(state, move_mgr, 100, 0.0)
        ...
        while T > T_min and time.time() < deadline:
            ...
            for i in range(iter_max):
                ...
                nodes, strategy_name = get_smart_move(state, move_mgr, T, progress)
                if not nodes: continue
                
                strategy_stats[strategy_name][0] += 1
                ...
                if accept:
                    strategy_stats[strategy_name][1] += 1
                    val = state.get_score()
        ...
        # 【改动】：添加日志前缀
        print(f"[{dataset_name}] [Restart {restart_idx}] Best: {global_best_score:,.0f} | InitT: {T_init:.1f}", flush=True)
        print(f"[{dataset_name}]   -> Strategy Stats (Proposed / Accepted):", flush=True)
        for s, (prop, acc) in strategy_stats.items():
            rate = (acc / prop * 100) if prop > 0 else 0.0
            print(f"[{dataset_name}]      {s}: {prop} / {acc} ({rate:.1f}%)", flush=True)
